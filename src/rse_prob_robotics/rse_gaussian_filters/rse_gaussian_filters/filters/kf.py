...
class KalmanFilter:
	...
	def predict(self, u, dt):
		...
		# Normalize theta to (-pi, pi] to prevent unbounded angular drift
		self.mu[2] = _normalize_angle(self.mu[2])
	...
	def update(self, z, dt):
		...
		# Simplified form — equivalent to the Joseph form
		# (Sigma = (I - K C) Sigma (I - K C)^T + K Q K^T) when C is linear.
		# NOTE: parentheses are critical here!  ``(I - K @ C) @ Sigma``
		# is *not* the same as ``I - K @ C @ Sigma`` — the latter expands to
		# an identity-like matrix and the filter never converges.
