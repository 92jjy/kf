...
class ParticleFilterBaseNode(Node):
    def __init__(self, pf):
        ...
        # ---- 观测坐标系安装偏角（坐标系约定）----
        # frame_rotation_deg = 观测源(odom_raw)坐标系相对标准 odom 坐标系的
        # 安装偏角（active 旋转，绕 z 右手定则 CCW 为正）。施加该旋转将观测
        # 变换到 odom 系，与真值(/odom，始终在 odom 系，不旋转)对比。
        #   * Gazebo 仿真：relay 直接转发 /odom -> /odom_raw，同坐标系 -> 0°
